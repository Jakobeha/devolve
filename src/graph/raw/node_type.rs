...
impl<RuntimeCtx: 'static + ?Sized> NodeTypes<RuntimeCtx> {
    ...
    pub fn insert_fn0(&mut self, name: &str, node_type_fn: impl Fn(NodeTypeFnCtx<'_>) -> Result<NodeType<RuntimeCtx>, Box<dyn Error>> + Send + Sync + 'static) {
        if self.statics.contains_key(name) || self.fns.contains_key(name) {
            error!("node type function with name {} already inserted", name);
        }
        self.fns.insert(name.to_string(), Box::new(move |arg, result| {
            if arg.is_empty() {
                node_type_fn(result)
            } else {
    ...
    ///
    /// Logs an error and overrides if the name already exists. Use [NodeTypes::contains_fn] to check
    pub fn insert_fn(&mut self, name: &str, node_type_fn: impl Fn(&str, NodeTypeFnCtx<'_>) -> Result<NodeType<RuntimeCtx>, Box<dyn Error>> + Send + Sync + 'static) {
        if self.statics.contains_key(name) || self.fns.contains_key(name) {
            error!("node type with name {} already inserted", name);
        }
        self.fns.insert(name.to_string(), Box::new(node_type_fn));
    }
