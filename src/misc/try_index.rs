...
impl<Idx1, Idx2: From<Idx1>> From<NotFound<Idx2>> for NotFound<Idx1> {
    fn from(not_found: NotFound<Idx2>) -> Self {
        NotFound {
            index: Idx1::from(not_found.index)
        }
    }
}
...
impl<T> TryIndex<usize> for Vec<T> {
    ...
    fn get(&self, index: usize) -> Result<&Self::Output, NotFound<Idx>> {
        self.get(index).ok_or(NotFound(index))
    }
}

impl<T> TryIndexMut<usize> for Vec<T> {
    fn get_mut(&mut self, index: usize) -> Result<&mut Self::Output, NotFound<Idx>> {
        self.get_mut(index).ok_or(NotFound(index))
    }
}
